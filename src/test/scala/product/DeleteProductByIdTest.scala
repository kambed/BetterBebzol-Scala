...
class DeleteProductByIdTest extends BaseTest {

  "Delete product by id" should "succeed with ok 200" in {
    ...
    Delete(s"/api/v1/product/${product_id.get}") ~> addHeader("Authorization", s"Bearer ${token.get}") ~> routes.get ~> check {
      status shouldEqual StatusCodes.OK
      responseAs[Map[String, String]].keySet should contain("product_id")
    }
  }
  
  "Try to delete product by id when not logged in" should "fail with unauthorized 401" in {
    val expectedResponse = Map("message" -> "Unauthorized")
    Delete("/api/v1/product/1") ~> routes.get ~> check {
