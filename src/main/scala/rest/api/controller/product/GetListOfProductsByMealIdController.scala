...
class GetListOfProductsByMealIdController(implicit system: ActorSystem[_]) extends BaseAuthenticatedController {
  ...
  @Operation(summary = "Get list of products by meal id", tags = Array("product"),
    parameters = Array(
      new Parameter(name = "mealId", in = ParameterIn.QUERY, required = true, description = "Meal id", content = Array(new Content(schema = new Schema(implementation = classOf[Long]))))),
    responses = Array(
