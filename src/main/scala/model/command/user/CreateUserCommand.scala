...
case class CreateUserCommand(@Schema(example = "example@wp.pl", requiredMode = Schema.RequiredMode.REQUIRED)
                             email: String,
  ...
  def toUser: User = User(0, email, password, sex.map(UserSex.withName), age, height, weight, howActive.map(UserActivity.withName), goal.map(UserGoal.withName))
}
