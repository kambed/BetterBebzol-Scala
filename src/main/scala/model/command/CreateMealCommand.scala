...
case class CreateMealCommand(@Schema(example = "breakfast", requiredMode = Schema.RequiredMode.NOT_REQUIRED)
  ...
  private val mealTypeValues: Set[String] = MealTime.values.map(v => v.toString)
...
//  def toMeal: Meal = Meal(0, userId, MealTime.withName(mealType.get), date.get)

  def toMeal: Meal = Meal(0, userId, MealTime.withName(mealType.get), 0, 0, 0, 0, date.get)
}
